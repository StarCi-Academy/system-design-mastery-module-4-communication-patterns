# Flow 5 — read-service chết tạm thời rồi hồi sinh (luồng lỗi/biên · C#)

## 1. Dừng read-service

```bash
docker compose stop cqrs-read-service-cs
# Container ...-csharp-read Stopped
```
...
```bash
docker compose start cqrs-read-service-cs
...
PASS — write decoupled khỏi read; event không mất nhờ queue `durable`; read-service hồi sinh tiêu thụ nốt event tồn đọng, project vào Elasticsearch, queue về `0/0`, `c3003` truy vấn được. Eventual consistency + resilience của CQRS qua broker.
