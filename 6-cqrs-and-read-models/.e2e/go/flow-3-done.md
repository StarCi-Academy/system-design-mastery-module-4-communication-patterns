# Flow 3 — Cập nhật hồ sơ và quan sát read model hội tụ (Go)
...
PASS — event cập nhật được publish, read-service index lại document theo `id` cố định (idempotent), read model hội tụ về `Alice Updated`. Queue drain `0/0` (ack sau khi project thành công).
